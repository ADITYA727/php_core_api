...
if(sizeof($dots)>2 && $dots[0]!='www' && strlen($dots[1])>3){
	...
	// print_r($protocol.'://'.$_SERVER['HTTP_HOST'].'/ <br>');
		// print_r($protocol.'://'.$_SERVER['HTTP_HOST'].'/includes/lib/ <br>');
	
	  //exit();
	    define("ROOT_PATH", $_SERVER["DOCUMENT_ROOT"].'/'.$cur_dirname.'/');
	    define("BASE_URL", '');
	    define("SITE_URL",$protocol.'://'.$_SERVER['HTTP_HOST'].'/kvt_api/');
    	define("AJAX_URL",$protocol.'://'.$_SERVER['HTTP_HOST'].'/kvt_api/includes/lib/');
	}else{
