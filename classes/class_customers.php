...
class rzvy_customers{
	...
	public function count_all_rc($search){
		...
		if($search != ''){
			...
			$query = "select count(`c`.`id`) from `".$this->rzvy_customers."` as `c`, `".$this->rzvy_bookings."` as `b` where ((`c`.`firstname` like '%".$search."%' or `c`.`lastname` like '%".$search."%') or `c`.`email` like '%".$search."%' or `c`.`phone` like '%".$search."%' or (`c`.`address` like '%".$search."%' or `c`.`city` like '%".$search."%' or `c`.`state` like '%".$search."%' or `c`.`zip` like '%".$search."%' or `c`.`country` like '%".$search."%')) and `b`.`customer_id` = `c`.`id` ".$group_by_qry;

		}else{

			$query = "select count(`c`.`id`) from `".$this->rzvy_customers."` as `c`, `".$this->rzvy_bookings."` as `b` where `b`.`customer_id` = `c`.`id` ".$group_by_qry;
	...
	public function get_all_rc_detail($start, $end, $search, $column,$direction, $draw){
		...
		if($search != ''){
			...
			$query = "select `c`.`id`, `c`.`firstname`, `c`.`lastname`, `c`.`email`, `c`.`phone`, `c`.`address`, `c`.`city`, `c`.`state`, `c`.`zip`, `c`.`country`, `c`.`refferral_code` from `".$this->rzvy_customers."` as `c`, `".$this->rzvy_bookings."` as `b` where ((`c`.`firstname` like '%".$search."%' or `c`.`lastname` like '%".$search."%') or `c`.`email` like '%".$search."%' or `c`.`phone` like '%".$search."%' or (`c`.`address` like '%".$search."%' or `c`.`city` like '%".$search."%' or `c`.`state` like '%".$search."%' or `c`.`zip` like '%".$search."%' or `c`.`country` like '%".$search."%')) ".$group_by_qry." ".$order_by_qry." limit ".$start.", ".$end;
...
// 			$query = "select `c`.`id`, `c`.`firstname`, `c`.`lastname`, `c`.`email`, `c`.`phone`, `c`.`address`, `c`.`city`, `c`.`state`, `c`.`zip`, `c`.`country`, `c`.`refferral_code` from `".$this->rzvy_customers."` as `c`, `".$this->rzvy_bookings."` as `b` ".$group_by_qry." ".$order_by_qry." limit ".$start.", ".$end;
	...
	public function count_all_gc($search){
		...
		if($search != ''){
			...
			$query = "select count(`b`.`order_id`) from `".$this->rzvy_customer_orderinfo."` as `c`, `".$this->rzvy_bookings."` as `b` where ((`c`.`c_firstname` like '%".$search."%' or `c`.`c_lastname` like '%".$search."%') or `c`.`c_email` like '%".$search."%' or `c`.`c_phone` like '%".$search."%' or (`c`.`c_address` like '%".$search."%' or `c`.`c_city` like '%".$search."%' or `c`.`c_state` like '%".$search."%' or `c`.`c_zip` like '%".$search."%' or `c`.`c_country` like '%".$search."%')) and `b`.`order_id` = `c`.`order_id` and `b`.`customer_id` = '0' ".$group_by_qry;

		}else{

			$query = "select count(`b`.`order_id`) from `".$this->rzvy_customer_orderinfo."` as `c`, `".$this->rzvy_bookings."` as `b` where `b`.`order_id` = `c`.`order_id` and `b`.`customer_id` = '0' ".$group_by_qry;
	...
	public function get_all_gc_detail($start, $end, $search, $column,$direction, $draw){
		...
		if($search != ''){
			...
			$query = "select `c`.`order_id`, `c`.`c_firstname`, `c`.`c_lastname`, `c`.`c_email`, `c`.`c_phone`, `c`.`c_address`, `c`.`c_city`, `c`.`c_state`, `c`.`c_zip`, `c`.`c_country` from `".$this->rzvy_customer_orderinfo."` as `c`, `".$this->rzvy_bookings."` as `b` where ((`c`.`c_firstname` like '%".$search."%' or `c`.`c_lastname` like '%".$search."%') or `c`.`c_email` like '%".$search."%' or `c`.`c_phone` like '%".$search."%' or (`c`.`c_address` like '%".$search."%' or `c`.`c_city` like '%".$search."%' or `c`.`c_state` like '%".$search."%' or `c`.`c_zip` like '%".$search."%' or `c`.`c_country` like '%".$search."%')) and `b`.`order_id` = `c`.`order_id` and `b`.`customer_id` = '0' ".$group_by_qry." ".$order_by_qry." limit ".$start.", ".$end;

		}else{

			$query = "select `c`.`order_id`, `c`.`c_firstname`, `c`.`c_lastname`, `c`.`c_email`, `c`.`c_phone`, `c`.`c_address`, `c`.`c_city`, `c`.`c_state`, `c`.`c_zip`, `c`.`c_country` from `".$this->rzvy_customer_orderinfo."` as `c`, `".$this->rzvy_bookings."` as `b` where `b`.`order_id` = `c`.`order_id` and `b`.`customer_id` = '0' ".$group_by_qry." ".$order_by_qry." limit ".$start.", ".$end;
	...
	public function get_all_rc_appointments($start, $end, $search, $column, $direction, $draw){
		...
		$query = "select `b`.`order_id`, `c`.`cat_name`, `s`.`title`, `b`.`addons`, `b`.`booking_datetime`, `b`.`booking_status` , `p`.`payment_method`, `b`.`service_rate` from `".$this->rzvy_categories."` as `c`, `".$this->rzvy_services."` as `s`, `".$this->rzvy_bookings."` as `b`, `".$this->rzvy_payments."` as `p` where `b`.`cat_id` = `c`.`id` and `b`.`service_id` = `s`.`id` and `b`.`order_id` = `p`.`order_id` and `b`.`customer_id`='".$this->id."' ".$search_by_qry." ".$group_by_qry." ".$order_by_qry." limit ".$start.", ".$end;
	...
	public function update_profile_with_image(){
		...
		$query = "update `".$this->rzvy_customers."` set `firstname`='".$this->firstname."', `lastname`='".$this->lastname."', `phone`='".$this->phone."', `address`='".$this->address."', `city`='".$this->city."', `state`='".$this->state."', `country`='".$this->country."', `zip`='".$this->zip."', `image`='".$this->image."' where `id`='".$this->id."'";
	...
	public function update_profile_without_image(){
		...
		$query = "update `".$this->rzvy_customers."` set `firstname`='".$this->firstname."', `lastname`='".$this->lastname."', `phone`='".$this->phone."', `address`='".$this->address."', `city`='".$this->city."', `state`='".$this->state."', `country`='".$this->country."', `zip`='".$this->zip."' where `id`='".$this->id."'";
	...
	public function get_business_customers(){
		...
		$query = "select `c`.`id`, `c`.`firstname`, `c`.`lastname`, `c`.`email` from `".$this->rzvy_customers."` as `c`, `".$this->rzvy_bookings."` as `b` where `b`.`customer_id` = `c`.`id` group by `c`.`id`, `c`.`firstname`, `c`.`lastname`, `c`.`email`";
	...
	public function check_email_availability($customer_email){
		...
		$query = "select `id` from `".$this->rzvy_customers."` where `email`='".$this->email."' and `email`<>'".$customer_email."'";
