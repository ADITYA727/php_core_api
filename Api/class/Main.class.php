...
class Main {
	...
	/*BEGIN DATABASE CONNECTION FUNCTION WITH MYSQLI*/
		private function DBConnection(){
			...
			// $con = mysqli_connect('localhost','eighteen_KVTDB','eighteenKVTDB','eighteen_KVTDB');
			$con = mysqli_connect('localhost','root','','eighteen_KVTDB');
	...
	/*BEGIN SEND MAIL FUNCTION */
		public function SendMail($mailto,$subject,$message,$attachments=""){
			...
			$headers .= 'From: Kashi Vishwanath Temple <info@eighteenpixels.in>' . "\r\n";
		    $headers .= 'Cc: ayaz@18pixels.com' . "\r\n";
						...
						if($array['limit']>1){
							...
							if(in_array(strtolower($currentfile_extension),$allowedfiletypes)){
								...
								if($files['size'][$a]<$max_size){
									...
									if(@move_uploaded_file($files['tmp_name'], SITE_ROOT.$filename)){
